...
namespace mini {
  namespace common {
    ...
    struct MINI_INTERFACE vec_t { T t[N]; };
    ...
    // ------------------------------------------------------------------
    template<typename T>
    struct MINI_INTERFACE vec_t<T,1> {
      enum { dims = 1 };
      typedef T scalar_t;
    ...
    // ------------------------------------------------------------------
    template<typename T>
    struct MINI_INTERFACE vec_t<T,2> {
      enum { dims = 2 };
      typedef T scalar_t;
    ...
    // ------------------------------------------------------------------
    template<typename T>
    struct MINI_INTERFACE vec_t<T,3> {
      enum { dims = 3 };
      typedef T scalar_t;
    ...
    // ------------------------------------------------------------------
    template<typename T>
    struct MINI_INTERFACE MINI_ALIGN(16) vec3a_t : public vec_t<T,3> {
      inline vec3a_t() {}
    ...
    // ------------------------------------------------------------------
    template<typename T>
    struct MINI_INTERFACE vec_t<T,4> {
      enum { dims = 4 };
      typedef T scalar_t;
    ...
    {
      return v * mini::common::polymorphic::rsqrt(dot(v,v));
    }
    ...
    {
      return mini::common::polymorphic::sqrt(dot(v,v));
    }
