...
OPTIX_CLOSEST_HIT_PROGRAM(MeshCH)()
{
  ...
  unsigned int sbtGASIndex = optixGetSbtGASIndex();
  optixGetTriangleVertexData(gas, prd.primID, sbtGASIndex, 0.f, vtx);
  vec3f N = cross((vec3f)vtx[1]-(vec3f)vtx[0],(vec3f)vtx[2]-(vec3f)vtx[0]);
  if (dot(N,N) == 0.f) {
    prd.scale = 1.f;
  } else {
    N = normalize((vec3f)optixTransformPointFromObjectToWorldSpace(N));
    prd.scale = .3f+.7f*abs(dot(N,vec3f(optixGetWorldRayDirection())));
  }
#endif
}
...
OPTIX_RAYGEN_PROGRAM(renderFrame)()
{
  ...
  uint32_t rgba = make_rgba(owl::randomColor(prd.instID)*prd.scale);
